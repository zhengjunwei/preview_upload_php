    ...
    public function stream2Image() {  
        ...
        if (! empty ( $data )) {  
            ...
            if (@$fp = fopen ( $this->save_fullpath, 'w+' )) {  
                fwrite ( $fp, $data );  
                fclose ( $fp );  
                $baseurl = "http://" . $_SERVER ["SERVER_NAME"] . ":" . $_SERVER ["SERVER_PORT"] . dirname ( $_SERVER ["SCRIPT_NAME"] ) . '/mm/' . $this->save_name;  
...
$my_obj = new imageUpload('abc.jpeg','./mmm');
print_r($my_obj->stream2Image());